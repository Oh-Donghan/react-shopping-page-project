...
export default function Search() {
  ...
  const filterProducts = products.filter((item) => item.title.toLowerCase().includes(inputValue.toLowerCase()));
      ...
      {openUl && (
        <ul className="!fixed left-0 sm:!absolute sm:top-14 menu flex-nowrap dropdown-content w-full sm:w-64 max-h-96 shadow text-base-content overflow-y-auto overflow-x-hidden bg-white dark:bg-gray-600">
          {filterProducts.map((product) => (
            <li key={product.id}>
              <button type="button" className="text-left js-searchedItem" onClick={() => handleItemClick(product.id)}>
